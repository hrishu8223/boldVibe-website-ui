...
const AnimatedLogo: React.FC<{ themeColor: string }> = ({ themeColor }) => {
  return (
    <div className="relative w-10 h-10 md:w-12 md:h-12 flex items-center justify-center">
      ...
      <span className="relative z-10 font-extrabold text-black text-xs md:text-sm">B</span>
    </div>
  );
};
  ...
  return (
    <>
      <style>{`
        @keyframes rotateTicks { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
        .tick-orbit { position: absolute; inset: 0; animation: rotateTicks 6s linear infinite; }
        .tick { position: absolute; width: 12px; height: 18px; border-right: 4px solid #000; border-bottom: 4px solid #000; border-radius: 2px; transition: border-color 0.5s; }
        @media (min-width: 768px) { .tick { width: 14px; height: 22px; border-right: 5px solid #000; border-bottom: 5px solid #000; } }
        .tick.top { top: 2px; left: 50%; transform: translateX(-50%) rotate(45deg); }
        .tick.right { right: 2px; top: 50%; transform: translateY(-50%) rotate(135deg); }
        .tick.bottom { bottom: 2px; left: 50%; transform: translateX(-50%) rotate(225deg); }
        .tick.left { left: 2px; top: 50%; transform: translateY(-50%) rotate(315deg); }
        ...
        <div className="max-w-7xl mx-auto px-4">
          <nav className="flex items-center justify-between h-16 md:h-20 gap-2">
            ...
            <Link to="/" className="flex items-center gap-2 md:gap-3 shrink-0">
              <AnimatedLogo themeColor={selectedCountry.color} />
              <span className="font-bold text-base md:text-lg hidden xs:block">
                <span className="text-black">BOLD</span>
                <span style={{ color: selectedCountry.color }} className="transition-colors duration-500 ml-1">
                  VIBE
                </span>
              </span>
            </Link>
            ...
            {/* UNIVERSAL COUNTRY SELECTOR (Mobile + Desktop) */}
            <div ref={dropdownRef} className="relative flex-1 flex justify-end">
              <button
                onClick={() => setDropdownOpen(!dropdownOpen)}
                className="flex items-center gap-2 md:gap-3 px-2 md:px-4 py-1.5 md:py-2 rounded-xl border-2 transition-all duration-500 hover:shadow-md"
                style={{ 
                  borderColor: selectedCountry.border,
                  backgroundColor: selectedCountry.bg,
                  color: selectedCountry.color 
              ...
              >
                ...
                <div className="flex items-center gap-1 md:gap-2 text-[10px] md:text-[13px] font-bold uppercase tracking-wide">
              ...
              {/* DROPDOWN MENU */}
              {dropdownOpen && (
                <div className="absolute top-full right-0 mt-2 w-56 md:w-64 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden z-[60] animate-in fade-in slide-in-from-top-2">
                  <div className="p-2 flex flex-col gap-1">
                    {countries.map((c) => {
                      const isSelected = selectedCountry.name === c.name;
                      return (
                        <button
                          key={c.name}
                          onClick={() => { setSelectedCountry(c); setDropdownOpen(false); }}
                          className="flex items-center justify-between px-3 py-2 md:px-4 md:py-3 rounded-xl transition-all hover:bg-gray-50 group text-left"
                          style={{ backgroundColor: isSelected ? c.bg : '' }}
                        >
                          <div className="flex items-center gap-3">
                            <span className="text-lg md:text-xl">{c.flag}</span>
                            <span className="text-xs md:text-sm font-bold transition-colors" style={{ color: isSelected ? c.color : '#374151' }}>
                              {c.name}
                            </span>
                          </div>
                          <span className="text-[9px] md:text-[10px] font-mono font-bold text-gray-400 group-hover:text-black">
                            {getFormatTime(c.timezone)}
                          </span>
                        </button>
                      );
            ...
            {/* Mobile Menu Toggle */}
            <button onClick={() => setIsOpen(true)} className="lg:hidden text-black p-2 ml-1">
              <Menu size={24} />
            </button>
          </nav>
        </div>
      ...
      <div className={`fixed inset-0 z-[100] transition-all duration-500 ${isOpen ? "visible" : "invisible"}`}>
        ...
        <aside className={`absolute right-0 top-0 h-full w-[80%] max-w-[300px] bg-white shadow-2xl transition-transform duration-500 ease-out transform ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
          <div className="flex flex-col h-full p-6">
            <div className="flex items-center justify-between mb-8">
              <span className="font-bold text-lg">Menu</span>
              <button onClick={() => setIsOpen(false)} className="p-2 bg-gray-100 rounded-full text-black"><X size={20} /></button>
            </div>
            <nav className="flex flex-col gap-4">
              {navLinks.map((link) => (
                <Link key={link.name} to={link.href} onClick={() => setIsOpen(false)} className="text-lg font-bold text-gray-800 hover:opacity-60 transition border-b border-gray-50 pb-2">
                  {link.name}
                </Link>
              ))}
            </nav>
